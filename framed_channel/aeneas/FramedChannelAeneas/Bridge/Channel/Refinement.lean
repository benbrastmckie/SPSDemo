...
/-!
# Bridge/Channel/Refinement: the extracted channel operations, once over any queue record

`[EXTRACTED: aeneas + bridge]` -- per-operation simulation of `rust/src/channel.rs`'s
`send`, `deliver`, `take` and `queued`, as extracted, against the specification channel
`Channel.send`/`Channel.deliver` at `Chan (Ext S Q (Vec U8) inst R)`.

## Stated once, over any lawful record

Charon and Aeneas turn the channel's `Q: BoundedQueue<Frame>` bound into an explicit record
...
functional relation `R` (`hfun`). None names the ring buffer or the list-backed queue: the queue
is reached only through `hsim`'s fields and `Bridge/Queue/Instance.lean`'s `extToList_eq` and
`extCapacity_eq`. The frame codec is reached only through `Frame.lean` beside this file.
`Instance.lean` then instantiates each consequence at both extracted queues.

## The divergence carried by `deliver`

`deliver_refines` inherits `ParsePost`'s case split: when the frame at the head of the wire
declares a length of `2 ^ 32` or more, the extraction refuses it where the model would try to read
it. No wire the channel wrote contains such a frame.
-/
...
* `hw`: room on the wire for the frame. Aeneas bounds a `Vec` by `Usize.max` and panics beyond it
  ...
  that bound. `[EXTRACTED: aeneas + bridge]` -/
theorem send_refines (c : channel.Channel S) (m : Chan (Ext S Q (alloc.vec.Vec Std.U8) inst R))
  ...
  by_cases hge : (QueueModel.toList (Q := Q) (α := alloc.vec.Vec Std.U8) q).length + m.inFlight ≥ QueueModel.capacity (Q := Q) (α := alloc.vec.Vec Std.U8) q
  · have hmf : FramedChannel.Channel.send (E := alloc.vec.Vec Std.U8) m (bitsOf payload.val) = .fail := by
      unfold FramedChannel.Channel.send; rw [if_pos (by rw [hlist, hcap]; exact hge)]
  ...
  · have hlt' : ¬ i1 ≥ k := by scalar_tac
    ...
    · have hmf : FramedChannel.Channel.send (E := alloc.vec.Vec Std.U8) m (bitsOf payload.val) = .fail := by
        unfold FramedChannel.Channel.send
    ...
    · have hl : payload.len.val ≤ U32.max := by simp [bitsOf] at hlong; scalar_tac
      ...
      step with encode_frame_refines payload len c.wire hlen hw as ⟨v, hv⟩
      step*
      have hmo : FramedChannel.Channel.send (E := alloc.vec.Vec Std.U8) m (bitsOf payload.val) =
          .ok { m with wire := m.wire ++ encodeFrame (bitsOf payload.val), inFlight := m.inFlight + 1 } := by
        unfold FramedChannel.Channel.send
...
theorem take_refines (c : channel.Channel S) (m : Chan (Ext S Q (alloc.vec.Vec Std.U8) inst R))
  ...
  · obtain ⟨p, hp, hpe⟩ := (WP.spec_equiv_exists _ _).mp (hsim.pop_empty c.out q hq' he)
    have hext : QueueModel.pop (Q := Ext S Q (alloc.vec.Vec Std.U8) inst R) (α := alloc.vec.Vec Std.U8) m.out = .fail := by
      show extPop m.out = _
    ...
    rw [hext]
    exact ⟨rfl, rfl⟩
  · obtain ⟨p, hp, y, q', hy, hpop, hR'⟩ := (WP.spec_equiv_exists _ _).mp (hsim.pop_ok c.out q hq' he)
    obtain ⟨o, s'⟩ := p
    simp only at hy hR'
    subst hy
    have hext : QueueModel.pop (Q := Ext S Q (alloc.vec.Vec Std.U8) inst R) (α := alloc.vec.Vec Std.U8) m.out =
        .ok (y, ⟨s', q', hR'⟩) := by
      show extPop m.out = _
...
theorem queued_agrees (c : channel.Channel S) (m : Chan (Ext S Q (alloc.vec.Vec Std.U8) inst R))
    ...
    channel.Channel.queued inst c ⦃ n =>
      n.val = (QueueModel.toList (Q := Ext S Q (alloc.vec.Vec Std.U8) inst R) (α := alloc.vec.Vec Std.U8) m.out).length ⦄ := by
  obtain ⟨hout, -, -⟩ := hR
  obtain ⟨q, hq⟩ := m.out.2
  unfold channel.Channel.queued
...
(`ofFrame_bitsOf`). `[EXTRACTED: aeneas + bridge]` -/
theorem deliver_refines (c : channel.Channel S) (m : Chan (Ext S Q (alloc.vec.Vec Std.U8) inst R))
    ...
    channel.Channel.deliver inst c ⦃ r c' =>
      ...
      (FramedChannel.Channel.deliver (E := alloc.vec.Vec Std.U8) m = .fail → r = .Err () ∧ c' = c) ⦄ := by
  obtain ⟨hout, hwire, hfl⟩ := hR
  obtain ⟨q, hq⟩ := m.out.2
  have hq' : R c.out q := hout ▸ hq
  have hderef : (alloc.vec.Vec.deref c.wire).val = c.wire.val := by
    simp [alloc.vec.Vec.deref, alloc.vec.Vec.val]
  unfold channel.Channel.deliver
  step with parse_frame_refines as ⟨o, hpost⟩
  rw [hderef, hwire] at hpost
  have hdeliver : ∀ p rest, parseFrame m.wire = .ok (p, rest) →
      FramedChannel.Channel.deliver (E := alloc.vec.Vec Std.U8) m =
        (match QueueModel.push m.out (FrameCarrier.ofFrame (E := alloc.vec.Vec Std.U8) p) with
         ...
         | .fail => .fail) := by
    intro p rest h
    unfold FramedChannel.Channel.deliver
    simp only [h]
  ...
  · simp only [ParsePost] at hpost
    ...
    · unfold FramedChannel.Channel.deliver at hm'
      rw [hf] at hm'
      cases hm'
    · exact Or.inr hwide
  · simp only [ParsePost] at hpost
    obtain ⟨hused, hparse⟩ := hpost
    have hcl : alloc.vec.CloneVec.clone core.clone.CloneU8 payload = ok payload := cloneVecU8_isId payload
    step*
    rw [hcl]
    simp only [bind_tc_ok]
    ...
    · obtain ⟨p, hp, hpe⟩ := (WP.spec_equiv_exists _ _).mp (hsim.push_full c.out q payload hq' hfull)
      have hext : QueueModel.push (Q := Ext S Q (alloc.vec.Vec Std.U8) inst R) m.out payload = .fail := by
      ...
      rw [hmp] at hm'
      cases hm'
    · obtain ⟨p, hp, hok, q', hpush, hR'⟩ := (WP.spec_equiv_exists _ _).mp (hsim.push_ok c.out q payload hq' hfull)
      obtain ⟨e, t⟩ := p
      simp only at hok hR'
      subst hok
      have hext : QueueModel.push (Q := Ext S Q (alloc.vec.Vec Std.U8) inst R) m.out payload = .ok ⟨t, q', hR'⟩ := by
        show extPush m.out payload = _
...
#print axioms FramedChannel.Bridge.channel.send_refines
#print axioms FramedChannel.Bridge.channel.take_refines
#print axioms FramedChannel.Bridge.channel.queued_agrees
#print axioms FramedChannel.Bridge.channel.deliver_refines
